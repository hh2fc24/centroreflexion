    ...
    <div className="min-h-screen bg-gray-50 py-16 sm:py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto mb-14 max-w-3xl animate-in fade-in slide-in-from-bottom-8 text-center duration-1000 sm:mb-20">
          ...
          <div className="mx-auto mt-8 mb-6 h-1 w-20 bg-red-700"></div>
            ...
            >
              ...
              <span
                className={`ml-2 rounded-full px-1.5 py-0.5 text-[10px] font-bold ${
                  activeCategory === "Todas" ? "bg-white/20 text-white" : "bg-red-100 text-red-600"
                }`}
              >
                {allArticles.length}
              </span>
          ...
          {activeCategory !== "Todas" ? (
            <p className="mt-4 text-sm text-gray-400">
              ...
              <strong className="text-gray-600">{activeCategory}</strong> ·{" "}
              <button onClick={() => setActiveCategory("Todas")} className="font-medium text-red-600 hover:underline">
                Ver todas
              </button>
            </p>
          ) : null}
          ...
          <div className="relative isolate mb-16 overflow-hidden rounded-2xl bg-white pr-0 ring-1 ring-gray-200 shadow-xl shadow-gray-200/50 sm:mb-24 sm:rounded-3xl lg:flex lg:items-center lg:pr-10 lg:pl-0">
            ...
            <div className="p-6 sm:p-10 lg:w-1/2 lg:pl-16">
              <div className="mb-5 flex flex-wrap items-center gap-3 text-sm sm:mb-6">
                <time dateTime={featuredArticle.date} className="font-medium uppercase tracking-wide text-gray-500">
                  {featuredArticle.date}
                </time>
                <span className="relative z-10 rounded-full bg-red-50 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-red-700">
                  {featuredArticle.category}
                </span>
              </div>
              <h3 className="mb-5 text-2xl font-bold leading-[1.2] text-gray-900 transition-colors hover:text-red-700 sm:mb-6 sm:text-4xl font-serif">
                    ...
                    <>
                      {details?.image ? (
                        <Image
                          ...
                          width={48}
                          height={48}
                          className="h-12 w-12 rounded-full object-cover shadow-md shadow-gray-300"
                        />
                      ) : (
                        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gray-900 text-sm font-bold text-white shadow-md shadow-gray-300">
                          {featuredArticle.author.charAt(0)}
                        </div>
                      )}
                      <div className="text-base leading-6">
                        <p className="font-semibold text-gray-900">{featuredArticle.author}</p>
                        {details?.role ? <p className="text-sm text-gray-500">{details.role}</p> : null}
                      </div>
                    </>
                  );
                })()}
        ...
        >
          {remainingArticles.map((post) => (
            <MotionItem
              key={post.id}
              className="group flex cursor-pointer flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100 transition-all duration-300 hover:shadow-lg hover:shadow-gray-200/50"
            >
              ...
              <div className="flex flex-1 flex-col justify-between p-5 sm:p-8">
                <div>
                  ...
                  <div className="group relative">
                    ...
                    <p className="line-clamp-3 text-sm leading-relaxed text-gray-600 sm:text-base">{post.excerpt}</p>
                  </div>
                </div>
                    ...
                    return (
                      <>
                        {details?.image ? (
                          ...
                          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-50 text-xs font-bold text-gray-600 ring-1 ring-gray-200">
                            {post.author.charAt(0)}
                          </div>
                        )}
                        <div className="flex-1 text-sm leading-6">
                          <p className="font-semibold text-gray-900">{post.author}</p>
                          {details?.role ? <p className="line-clamp-1 text-xs text-gray-500">{details.role}</p> : null}
                        </div>
                      </>
                    );
                  })()}
        ...
        {filteredArticles.length === 0 ? (
          <div className="py-16 text-center">
            <p className="text-lg text-gray-400">No hay artículos en esta categoría.</p>
            <button onClick={() => setActiveCategory("Todas")} className="mt-4 font-semibold text-red-600 hover:underline">
