...
import {
  findDesproteccionEventRegistrationById,
  readDesproteccionEventRegistrations,
} from "@/lib/server/eventRegistrations";
...
export async function GET(req: Request) {
  ...
  try {
    if (id) {
      const result = await findDesproteccionEventRegistrationById(id);
      ...
      return NextResponse.json({ ok: true, found: false }, { headers: NO_STORE_HEADERS });
    }
